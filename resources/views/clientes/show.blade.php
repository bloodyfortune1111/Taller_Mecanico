           ...
           <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
               <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                   <div class="p-6 text-gray-900">
                       <h3 class="text-lg font-medium text-gray-900 mb-4">Detalles del Cliente: {{ $cliente->nombre }} {{ $cliente->apellido }}</h3>

                       <div class="border-t border-gray-200 pt-4">
                           <dl class="divide-y divide-gray-200">
                               <div class="px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                                   <dt class="text-sm font-medium leading-6 text-gray-900">ID</dt>
                                   <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">{{ $cliente->id }}</dd>
                               </div>
                               <div class="px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                               ...
                               <div class="px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                                   ...
                                   <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">{{ $cliente->apellido }}</dd>
                               </div>
                               <div class="px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                               ...
                               <div class="px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                                   ...
                                   <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">{{ $cliente->created_at->format('d/m/Y H:i') }}</dd>
                               </div>
                               <div class="px-4 py-3 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                                   <dt class="text-sm font-medium leading-6 text-gray-900">Última Actualización</dt>
                                   <dd class="mt-1 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0">{{ $cliente->updated_at->format('d/m/Y H:i') }}</dd>
                               </div>
                           </dl>
                       </div>

                       <div class="mt-6 flex items-center justify-end gap-x-6">
                           <a href="{{ route('clientes.edit', $cliente->id) }}" class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
                               Editar Cliente
                           </a>
                           <form action="{{ route('clientes.destroy', $cliente->id) }}" method="POST" class="inline">
                               @csrf
                               @method('DELETE')
                               <button type="submit" class="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600" onclick="return confirm('¿Estás seguro de eliminar este cliente?')">
                                   Eliminar Cliente
                               </button>
                           </form>
                           <a href="{{ route('clientes.index') }}" class="px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400">
                               Volver a la Lista
                           </a>
                       </div>
                   </div>
